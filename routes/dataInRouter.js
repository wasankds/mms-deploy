...
// 
// - ถ้าไม่ลงทะเบีนรอุปกรณ์ไว้ จะไม่สามารถส่งข้อมูลมาได้
// 
router.post(/^\/e\d{3}$/, async (req, res) => {
  ...
  try{
    //=== 1.) ค้นหา key ใน global.DATA_DEVICES
    const deviceFind = global.DATA_DEVICES.find( dev => dev.deviceId === id && dev.deviceKey === key )
    if(!deviceFind){
      return res.status(403).send({ status : 'error', msg: 'Incorrect key or device ID' });
    }
