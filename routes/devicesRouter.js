...
router.get(PATH_MAIN, mainAuth.isOA, async (req, res) => {
  ...
  try {
    ...
    if (load_id) {
      ...
      //= 2.1.2) ถ้าไม่พบเอกสาร
      if (deviceToLoad) {
        const deviceImagePath = path.join(global.folderDevices, `${deviceToLoad.deviceId}.jpg`)
        try {
          if (fs.existsSync(deviceImagePath)) {
            const imageData = await fs.promises.readFile(deviceImagePath);
            const base64Image = imageData.toString('base64');
            deviceToLoad.deviceImageBase64 = `data:image/jpeg;base64,${base64Image}`;
          }
        } catch (err) {
...
const upload = multer({ 
  storage: multer.diskStorage({
    destination: async function (req, file, cb) { // Make the destination function async
      try {
        await fs.promises.mkdir(global.folderDevices, { recursive: true }); // Use async mkdir
        cb(null, global.folderDevices);
      } catch (err) {
        cb(err); 
      }
    },
    ...
    filename: function (req, file, cb) {
      ...
      //=== ตั้งชื่อไฟล์ตาม deviceId ที่ส่งมา
      const deviceId = req.body.deviceId
      cb(null, deviceId + path.extname(file.originalname));
      ...
      // const deviceId = req.body.deviceId
      // const regex = new RegExp(DEVICE_PATTERN)
      // if (!regex.test(deviceId)) {
      //   cb(new Error('ไอดีไม่ถูกต้อง'))
      // }else{
      //   //=== ตั้งชื่อตามไอดี
      //   const fileExtension = path.extname(file.originalname);
      //   const filename = `${deviceId}${fileExtension}`; // Use _id as filename
      //   cb(null, filename);
      // }
...
router.post(PATH_SAVE, [
    ...
    ( req, res, next) => { // 2 - อัปโหลดไฟล์ (เขียนแบบนี้เพราะต้องการ handleMulterError)
      upload(req, res, async (err) => {
        ...
        //=== resize ภาพต้นฉบับที่อัปโหลดมา 
        // - ขนาดกว้างไม่เกิน 900px สูงตามสัดส่วน และบันทึกเป็น .jpg เสมอ
        if (req.file) {
          ...
          try { // อาจติด lock ต้องครอบ try catch
  ...
  const deviceId = req.body.deviceId
  // แปลงชนิดข้อมูล
  req.body.deviceLatitude = req.body.deviceLatitude ? Number(req.body.deviceLatitude) : null
  req.body.deviceLongitude = req.body.deviceLongitude ? Number(req.body.deviceLongitude) : null
  ...
  if (!global.DEVICE_REGEX.test(deviceId)) {
  ...
  try {
    ...
    //=== 2.) กรณี Update - ******************************      
      ...
      if (rtnUpdate.acknowledged && rtnUpdate.modifiedCount == 1) {
        ...
        //= 2.4.2 จับข้อมูลใส่ gloval.DATA_DEVICES ใหม่
...
router.post(PATH_DELETE, mainAuth.isOA, async (req, res) => {  
  ...
  try {
    ...
    if (deleteResult.deletedCount === 1) {
      ...
      //== 2.2) จับข้อมูลใส่ gloval.DATA_DEVICES ใหม่
      global.DATA_DEVICES = await myData.getDataDevices()
